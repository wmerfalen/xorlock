...
namespace air_support::f35 {
  ...
  static std::deque<Actor> bomb_targets;
#define MODE_IDLE 0
...
#define MODE_DISPATCH_NOW 3
  ...
  void draw_target(const Actor& p){
    if(halt_f35){
      return;
    }
    draw::blatant_rect(&p.rect);
  }
  ...
  static inline void cast_rays(){
    ...
    for(const auto& p : target_offsets){
      SDL_Rect r{x + p.first,y + p.second,50,50};
      draw::blatant_rect(&r);
    }
  }
  static inline std::deque<Actor> save_bomb_targets(){
    if(halt_f35){
      return {};
    }
  ...
  bool ready(){
    return mode == MODE_IDLE && accept_space_bar;
  }
  uint32_t detonate_targets(uint32_t interval, void* ignored){
    ...
    LOCK_MUTEX(bomb_targets_mutex);
    for(const auto& target : bomb_targets){
