...
namespace bullet {
  ...
  void Bullet::calc() {
    ...
    void Bullet::travel() {
      ...
      if(queue_type == QUEUE_TYPE_PLAYER || queue_type == QUEUE_TYPE_TURRET){
        for(auto& npc : world->npcs()) {
          if(npc->z < 0){ // TODO: replace 0 with attacker's z value
            continue;
          }
    ...
    void draw_shell_at(const uint32_t& x,const uint32_t& y,const uint32_t& wpn_type){
      ...
      shell_positions[shell_positions_index].where.rect.x = x;
      shell_positions[shell_positions_index].where.rect.y = y;
      shell_positions[shell_positions_index].where.rect.w = 22;
      shell_positions[shell_positions_index].where.rect.h = 8;
        ...
        if(is_shotgun(shell.type)){
          if(shell.velocity > 0){
            ...
            switch(shell.trajectory){
              case SOUTH:
                shell.where.rect.y += rand_between(start,end);
                if(rng::chaos()){
                  shell.where.rect.x += rand_between(start,end);
                }
                if(rng::chaos()){
                  shell.where.rect.x -= rand_between(start,end);
                }
            ...
            if(shell.trajectory_change_at + 30 < i){
              for(const auto& w : wall::blockable_walls){
                if(SDL_IntersectRect(&shell.where.rect,&w->self.rect,&r)){
                  shell.trajectory_change_at = tick::get();
                  switch(shell.trajectory){
                    case NORTH:
                      if(w->self.rect.y < shell.where.rect.y){
                        if(rng::chaos()){
                      ...
                      break;
                    case NORTH_EAST:
                      if(w->self.rect.y < shell.where.rect.y || w->self.rect.x > shell.where.rect.x){
                        shell.trajectory = SOUTH_EAST;
                      }else{
                        shell.trajectory = SOUTH_WEST;
                      }
    ...
    void init() {
      ...
      pool = std::make_unique<BulletPool>();
      shell_positions_index = 0;
      for(size_t i=0; i < MAX_SHELL_POSITIONS; i++){
        register_actor(&shell_positions[i].where);
      }
    }
    void cleanup_pool() {
    }
