...
static void output_test_warnings(void)
{
    ...
    printf("Warning: TEST_MIX_CHANNELFINISHED is enabled in this binary...\n");
#endif
#if (defined TEST_MIX_PANNING)
    printf("Warning: TEST_MIX_PANNING is enabled in this binary...\n");
#endif
#if (defined TEST_MIX_VERSIONS)
    printf("Warning: TEST_MIX_VERSIONS is enabled in this binary...\n");
#endif
#if (defined TEST_MIX_DISTANCE)
    printf("Warning: TEST_MIX_DISTANCE is enabled in this binary...\n");
#endif
#if (defined TEST_MIX_POSITION)
    printf("Warning: TEST_MIX_POSITION is enabled in this binary...\n");
#endif
}
...
{
    printf("This program was compiled against %s %d.%d.%d,\n"
...
static void do_panning_update(void)
{
    ...
    if ((panningok) && (SDL_GetTicks() >= next_panning_update)) {
        ...
        if (!panningok) {
            printf("Mix_SetPanning(0, %d, %d) failed!\n",
...
static void do_distance_update(void)
{
    ...
    if ((distanceok) && (SDL_GetTicks() >= next_distance_update)) {
        ...
        if (!distanceok) {
            printf("Mix_SetDistance(0, %d) failed!\n", (int) distance);
            printf("Reason: [%s].\n", Mix_GetError());
        }
...
static void do_position_update(void)
{
    ...
    if ((positionok) && (SDL_GetTicks() >= next_position_update)) {
        ...
        if (!positionok) {
            printf("Mix_SetPosition(0, %d, %d) failed!\n",
...
{
    printf("Usage: %s [-8] [-f32] [-r rate] [-c channels] [-f] [-F] [-l] [-m] <wavefile>\n", argv0);
}
...
static void flip_sample(Mix_Chunk *wave)
{
    ...
    switch (incr) {
        ...
        default:
            printf("Unhandled format in sample flipping.\n");
            return;
    }
}
